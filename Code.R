...
# Also, make sure that you have completed (or restored) stockmarket4
...
#custom calendar
qry<-"SELECT * FROM custom_calendar2 WHERE date BETWEEN '2015-12-31' AND '2021-03-26' ORDER by date"
ccal<-dbGetQuery(conn,qry)
#eod prices and indices - CHECK DATE WITH PROFESSOR?
qry1="SELECT symbol,date,adj_close FROM eod_indices WHERE date BETWEEN '2015-12-31' AND '2021-03-26'"
qry2="SELECT ticker,date,adj_close FROM eod_quotes WHERE date BETWEEN '2015-12-31' AND '2021-03-26'  and ticker in ('JNJ', 'JOUT', 'JPM','LIQT','LNTH','LOGI','MA','MASI','MELI','SGU','SMP','SYF','VAC','VEEV','VRNS')"
...
write.csv(eod_ret,'C:/Temp/eod_ret2.csv')
...
# We will select 'SP500TR' and 15 ASSINGED TICKERS
selectedtickers <- c('JNJ', 'JOUT', 'JPM','LIQT','LNTH','LOGI','MA','MASI','MELI','SGU','SMP','SYF','VAC','VEEV','VRNS')
# We need to convert data frames to xts (extensible time series)
Ra<-as.xts(eod_ret[,selectedtickers,drop=F])
...
opt_w<-round(opt_p$weights,digits = 4)
